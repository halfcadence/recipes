...
- **A `:not(:has(…))` selector MUST NOT be nested under a parent selector.** GitHub Pages builds `main.scss` with jekyll-sass-converter 1.5.2 → **Ruby Sass 3.7.4**, which predates `:has()`. At the top level it passes the selector through untouched; nested (or written with `&`) it has to re-parse to prepend the parent, fails, and emits the rule with the whole `:not(…)` **silently deleted** — so the rule then matches everything. Exit 0, green build, no warning. This shipped: the no-Analysis notes rule went live unconditional and 70 recipes printed their notes on top of the "analysis" head. Write the full selector out at the top level of its media block, or build it from an interpolated string variable (interpolation is textual, never re-parsed). Run `python3 bin/check-selectors.py` before pushing any stylesheet change — and remember the general rule it stands for: **a local Sass compile is not evidence about production.** Dart Sass and libsass both accepted what Pages threw away, so verify against the live CSS or against Ruby Sass 3.7.4 itself (command is in the script's docstring).
