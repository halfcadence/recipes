...
| 100g | sugar |
| 50g | cornstarch |
| 10g | vanilla bean paste (or 2 vanilla beans, split and scraped — see notes) |
| | pinch of salt |
...
3. **Custard (one pot, no tempering):** in a saucepan off the heat, whisk the milk, eggs, yolks, sugar, cornstarch, vanilla, and salt together cold until completely smooth with no cornstarch streaks. (Using beans instead of paste? Scrape in the seeds and drop in the split pods too — fish the pods out before pouring.)
...
|---|---|---|
| Structure solids (cornstarch) | 50g | 4.8% |
| Fat (milk, egg, yolk) | 49g | 4.6% |
| Sugar (added + lactose) | 137g | 13.0% |
| Liquid (milk and egg water) | 768g | 73.4% |
| Egg/milk protein | 43g | 4.1% |

**A starch-and-egg set, which is the whole point:** unlike a pure egg-set purin (baked gently in a water bath and spoonable), the ~5% cornstarch is what lets this slice into clean wedges. The starch also protects the eggs — its granules block the egg proteins from cross-linking, so the custard can be boiled on the stove without curdling. It's boiled deliberately: egg yolks carry amylase, a starch-digesting enzyme, and only a full boil kills it — skip that and the starch is quietly digested as it cools and the flan slumps. Sugar reads modest (~13%) because there's no fruit bringing its own, just milk and added sugar.
...
- **Real vanilla bean, still one pot.** You don't need a separate steep to use a bean — the one-pot mix comes to a boil anyway, so just split and scrape 2 beans (seeds *and* pods) straight into the cold pot with everything else, heat, and fish out the pods before pouring (or let the sieve catch them). The seeds carry the flecks and most of the flavor and release fully. A dedicated 15-min hot steep pulls a little more out of the woody pods, but it means heating the milk separately and tempering — not worth it here. For a touch more, leave the pods in while the custard cools, then strain. Vanilla paste is the zero-effort route at ~90% of the flavor.
